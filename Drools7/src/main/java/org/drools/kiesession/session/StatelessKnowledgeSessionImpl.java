...
public class StatelessKnowledgeSessionImpl extends AbstractRuntime implements StatelessKieSession {
    ...
    public static final KieCommands KIE_COMMANDS = KieServices.get().getCommands();
    ...
    public void initMBeans(String containerId, String kbaseId, String ksessionName) {
        if (kBase.getConfiguration() != null && kBase.getKieBaseConfiguration().isMBeansEnabled() && mbeanRegistered.compareAndSet(false,
            true)) {
    ...
    public <T> T execute(Command<T> command) {
        ...
        try {
            if (command instanceof BatchExecutionCommand) {
                context.register(ExecutionResults.class, KIE_COMMANDS.newExecutionResults());
            }
    ...
    public void executeWithFilter(Iterable objects, AgendaFilter agendaFilter) {
        StatefulKnowledgeSession ksession = newWorkingMemory();
        try {
            for (Object object : objects) {
                ksession.insert(object);
